...
class Payresult extends CI_Controller {
	...
	public function index() {
		try{
			$myfile = fopen("/var/www/html/application/controllers/module/pchome/".$arFeedback["CustomerOrderID"].".txt", "w") or die("Unable to open file!");
			...
			if (sizeof($arFeedback) > 0) {
				if($arFeedback['Status']=='1'){  //
					//檢查訂單是否存在以及金額是否正確
					$sqlStr="select * from `orders` where `order_no`=? and `OrderID`=?";
					$parameter=array(':order_no'=>$arFeedback["CustomerOrderID"]);
