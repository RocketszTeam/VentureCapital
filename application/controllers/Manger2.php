...
class Manger2 extends Core_controller{
				...
				if(preg_match('/^[0-9]*[1-9][0-9]*$/',$this->input->post('amount'))){
					if($this->input->post('amount') <= (int)$this->data["user_info"]["WalletTotal"]){
						...
						if($order_no!=NULL){
							...
							$this -> session -> set_flashdata("alertMsg",'提款單已經送出，我們將盡快為您處理。');
	...
	public function transfer(){
		...
		if($logMsg!=NULL){
			...
			//遊戲廠商下拉
			$sqlStr="select * from `game_makers` where num in(3,4,8,9,11,12,20,21,22,23,24,26) order by `range`";	//排除黃金俱樂部 and 捕魚機
	...
	public function deposit(){
		...
		if($logMsg!=NULL){
			...
			$this -> load -> view("www/deposit2.php", $this -> data);
				...
				if($this->input->post('amount') && $this->input->post('payment')){
					...
					if(preg_match('/^[0-9]*[1-9][0-9]*$/',$amount)){
						...
						if($order_no!=NULL){
							...
							$this->webdb->sqlExc($sqlStr,$parameter);
							//取號
							$log=$this->esafe->getFormArray($order_no,$amount,$payment);
							if($log==NULL){
								scriptMsg("","Manger2/deposit_result?order_no=".$order_no);
								exit;
							}else{
								$this -> session -> set_flashdata("alertMsg",$log);
								scriptMsg('',"Manger2/deposit");
								exit;
							}
						}else{
							$this -> session -> set_flashdata("alertMsg",'訂單建立失敗‧‧請重新嘗試');
