...
class Super_report extends Core_controller{
	...
	*/
			...
			if($rowAll!=NULL){	//執行成功且有帳、資料回來
				foreach($result as $row){
					...
					for($i=0; $i<count($rowAll); $i++){	
						//$this->data[$k]=$v;
						if(strtolower($rowAll[$i]["u_id"]) == strtolower($row->m_id)){
					...
					$parameter=array();
					$colSql="sn,m_id,up_no1,up_no2,m_date,count_date,team_no,fashion,g_type";
	...
	public function update_report($sTime=NULL,$eTime=NULL){
		...
		if($sTime==NULL && $eTime==NULL){
			...
			//$eTime=date('Y-m-d H:i:s',strtotime($eTime."+1 day"));
		}
		$result=$this->superapi->reporter_all($sTime,$eTime);
