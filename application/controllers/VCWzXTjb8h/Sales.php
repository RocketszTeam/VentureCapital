...
class Sales extends Core_controller{
	...
	public function __construct(){
		...
		$this -> data["picDir"]=UPLOADS_PATH . '/active/';	//上傳目錄
	...
	public function index(){	
		...
		$parameter=array();
		$sqlStr="select * from `active` where 1=1";
		//---分類-------------------------------
		if(@$_REQUEST["find1"]!=""){	
			$sqlStr.=" and `kind`=?";	
		...
		//分頁相關
		$config['base_url'] = base_url().SYSTEM_URL."/Sales/index?".$this->data["att"];//site_url("admin/news/index");
		...
		//撈出分類查詢下拉用
		$parameter=array();
		$sqlStr="select * from `active_kind` where `root`=0 and `nation`=?";
		$parameter[":nation"]=$this->defaultNation();
		$sqlStr.=" order by `range`";
		$this->data["row_group"]=$this->webdb->sqlRowList($sqlStr,$parameter);
		
		$this->data["editBTN"]=SYSTEM_URL."/Sales/edit/";
		$this->data["delBTN"]=SYSTEM_URL."/Sales/delete/";
							
		$this -> data["body"] = $this -> load -> view("admin/sales/index", $this -> data,true);   
	...
	function create(){	//新增選項
		...
		if($_POST){	//新增資料
			...
			$sqlStr="INSERT INTO `active` (".sqlInsertString($colSql,0).") VALUES (".sqlInsertString($colSql,1).")";
			...
			if(!$this->webdb->sqlExc($sqlStr,$parameter)){
				...
				$this -> _setMsgAndRedirect($msg, SYSTEM_URL."/Sales/index");
			}
		}else{
			//撈出分類查詢下拉用
			$parameter=array();
			...
			$sqlStr.=" order by `range`";
			$this->data["row_group"]=$this->webdb->sqlRowList($sqlStr,$parameter);
			$this->data["formAction"]=site_url(SYSTEM_URL."/Sales/create");
			$this->data["cancelBtn"]=site_url(SYSTEM_URL."/Sales/index");
			$this->data["subtitle"]=$this->getsubtitle();	//取得當前頁面名稱
			$this->data["todo"]="add";
			$this -> data["body"] = $this -> load -> view("admin/sales/form", $this -> data,true); 
			$this -> load -> view("admin/main", $this -> data);	
		}
	...
	function edit($num){
		...
		$this->_append_js('ckeditor/ckeditor.js');
		//============================================
		$sqlStr="select * from `active` where num=?";
		...
		if($row==NULL){
			...
			if($_POST){	//修改資料
				...
				$sqlStr="UPDATE `active` SET ".sqlUpdateString($colSql);
				...
				$this->data["formAction"]=site_url(SYSTEM_URL."/Sales/edit/".$row["num"]);
				$this->data["todo"]="edit";
				$this->data["subtitle"]=$this->getsubtitle();	//取得當前頁面名稱
				$this->data["cancelBtn"]=site_url(SYSTEM_URL."/Sales/index");
				$this->data["row"]=$row;	
	...
	function delete($num){
		...
		if($row==NULL){
			...
			$sqlStr="delete from `active` where num=?";
