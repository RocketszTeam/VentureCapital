...
class Member extends Core_controller
{
    ...
    public function index()
    {
        ...
        $parameter = array();
        $query_info = $this->buildMemberInfoQuerySql($bt);
        $sqlStr = $query_info['sqlStr'];
        $parameter = $query_info['parameter'];
        //除了最高權限 能看到所有...否則只抓該使用者的分類
        if (in_array($this->web_root_u_power, array(4, 5, 6))) {
            $sqlStr .= " and m.`admin_num` in (?" . kind_sql($this->web_root_num, "admin") . ")";
            $parameter[":admin_num"] = $this->web_root_num;
        }

        $sqlStr .= " group by m.num ";
        ...
        if (@$_REQUEST["find12"] != "" && @$_REQUEST["find13"] != "") {
            if (@$_REQUEST["find12"] != "AllSave" && @$_REQUEST["find12"] != "AllSell" && @$_REQUEST["find12"] != "WeekSell")
                $sqlStr .= " order by m.`" . @$_REQUEST["find12"] . "` " . @$_REQUEST["find13"];
            else
                $sqlStr .= " order by `" . @$_REQUEST["find12"] . "` " . @$_REQUEST["find13"];
        } else {
            $sqlStr .= " order by m.`login_time` DESC ";
        }
        //更換排序的網址變數
        $att2 = "";
        foreach ($_REQUEST as $key => $value) {
            if ($key != "find12" && $key != "find13") {
                $att2 .= "&" . $key . "=" . $value;
            }
        }
        $this->data["att2"] = $att2;
        ...
        $sqlStr .= " LIMIT " . ((($nowpage > 0 ? $nowpage : 1) - 1) * $limit) . "," . $limit;
        ...
        $this->data["excel_auth"] = $this->chk_auth("Member/excel");
        ...
        if ($_POST) {    //新增資料
            if ($this->web_root_u_power != 4 && $this->web_root_u_power != 5) {    //不允許股東and 總代新增會員
                if ($this->chk_id($this->input->post("u_id", true))) {    //撿查帳號是否重複
                    ...
                    $parameter[":promoter"] = trim($this->input->post("promoter", true));
    ...
    function edit($num)
    {
        ...
        if ($row == NULL) {
            ...
            if ($_POST) {    //修改資料
                $parameter = array();
                $colSql = "nation,u_name,phone,line,email,active,is_vaid,demo,m_group,promoter";
            ...
            if ($_POST["gamemaker_num"] != "" && $_POST["mem_num"] != "" && $_POST["u_password"] != "") {
                ...
                if ($row == NULL) {
                    ...
                    if ($log == NULL) {
                        ...
                        $msg = array("type" => "success", 'title' => '創建完成！', 'content' => $log);
                        $this->_setMsgAndRedirect($msg, $this->agent->referrer());
                    }
                } else {
        ...
        if (!$this->agent->is_referral()) {    //判斷使用者是否別網站來的
            if ($this->input->is_ajax_request()) {    //檢查token是否正確
                if ($this->session->userdata('wallet_token') && $this->input->post('token') == $this->session->userdata('wallet_token')) {
					...
					//若point_type = "1" (放點)時 而且 active = "1" (開啟)，則會進入判斷式，
					//若point_type = "1" (放點)時 而且 active = "2" (關閉)，則放點無限制
					if($point_type=="1" && $this->input->post('ttp',true) =="1" && $this->input->post('tty',true) !=="0"){
						
						//取得"管理者轉入"預設點數
						$totalkindpoint = $this->input->post('tty',true);
						//取得目前該會員由管理者轉入總共為多少
						$totalkina1 = $this->input->post('ttt',true);
						
						//若管理者放點點數大於預設放點點數，則不可以放點
						if($totalkindpoint < $totalkina1){ 
							echo json_encode(array('RntCode'=>0,'title'=>'請求失敗','Msg'=>'已超過放點最大點數','blance'=>getWalletTotal($this->input->post('mem_num'))));
							exit;
						}
    ...
    function ajaxWalletKind()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post('point_type')) {
				...
				//取得目前該會員 錢包轉點與扣點的類型 總共為多少
				$kind1_sqlStr = "select sum(points) as totalpoints from member_wallet where kind=".$this->input->post('point_type')." and mem_num=".$this->input->post('mem_num');
				$kind1_rowAll = $this->webdb->sqlRow($kind1_sqlStr);
				//若已放點數為NULL 則顯示為0
				$totalkina1 =  ($kind1_rowAll['totalpoints'] == NULL ? 0 : $kind1_rowAll['totalpoints'] );
				//$totalkina1 =  $kind1_rowAll['totalpoints'];
				
                echo '<option value="">請選擇</option>';
                if ($rowAll != NULL) {

                    foreach($rowAll as $row){
						if($row["num"] == "1"){ //若有"管理者轉入"則顯示目前總共轉入多少前， 並加入option kind1屬性質
							echo '<option value="'.$row["num"].'" kind1="'.$totalkina1.'" kind1_point="'.$row["point"].'" active="'.$row["active"].'">'.$row["kind"].'.......最大限額:'.$row['point'].'，已放點數:'.$totalkina1.'</option>';
						}else{
    ...
    /**
     * 建立會員管理系統查詢SQL(會員資料查詢功能與匯出excel)
     * @param string $bt 日期
     * @return array
     */
    private function buildMemberInfoQuerySql($bt)
    {
        $parameter = array();
        $sqlStr = "select *,
		(select SUM(amount) from `member_sell` where `keyin1`=1 and `mem_num`=m.num) as AllSell,
		( IFNULL( (select SUM(amount) from `orders` where `keyin2`=1 and `mem_num`=m.num),0) + 
		IFNULL( (select SUM(amount) as MYAmount from `member_bank_transfer` where `keyin2`=1 and `mem_num`=m.num),0) ) as  AllSave,
		(select Count(mem_num) from `member_sell` where `keyin1`=1 and `mem_num`=m.num and `buildtime` >= '" . $bt . "') as WeekSell
		from `member` as m where 1=1";

        //---所屬代理-------------------------------
        if (@$_REQUEST["find1"] != "") {
            $sqlStr .= " and m.`admin_num`=?";
            $parameter[":admin_num"] = @$_REQUEST["find1"];
        }
        //---帳號-------------------------------
        if (@$_REQUEST["find2"] != "") {
            $sqlStr .= " and m.`u_id` like ?";
            $parameter[":u_id"] = "%" . trim(@$_REQUEST["find2"]) . "%";
        }
        //---姓名-------------------------------
        if (@$_REQUEST["find3"] != "") {
            $sqlStr .= " and m.`u_name` like ?";
            $parameter[":u_name"] = "%" . trim(@$_REQUEST["find3"]) . "%";
        }
        //---推廣-------------------------------
        if (trim(@$_REQUEST["find14"]) != "") {
            $sqlStr .= " and m.`promoter` like ?";
            $parameter[":promoter"] = "%" . trim(@$_REQUEST["find14"]) . "%";
        }
        ...
        if (@$_REQUEST["find4"] != "") {
            ...
            if ($rowIP != NULL) {
                ...
                $sqlStr .= " and m.num in ($ipList)";
            } else {
                $sqlStr .= " and m.num is null";
            }
        }
        //---電話-------------------------------
        if (@$_REQUEST["find5"] != "") {
            $sqlStr .= " and m.`phone` like ?";
