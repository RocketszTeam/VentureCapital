...
class Order_kind extends Core_controller{
	
	private $kindLevel=1;	//分類層數
	public function __construct(){
		parent::__construct();
		
		$this -> load -> model("sysAdmin/order_kind_model", "kind");
	...
	public function index($root = 0,$nation=NULL){		
		...
		$this->data["action"] = site_url("sysAdmin/order_kind/create/".(empty($root) ? "" : $root)."/".(empty($nation) ? "" : $nation));
		$this->data["subtitle"] = '新增頁面主分類';
		$this->data["maintitle"]=$this->getsubtitle();	//取得當前頁面名稱			
		$this -> data["body"] = $this -> load -> view("sysAdmin/order/kind", $this -> data,true);   
	...
	function create($root=0,$nation=NULL){	//新增選項	
		...
		if($_POST){	//新增資料
			...
			if(!$this->kind->sqlExc($sqlStr,$parameter)){
				...
				$this -> _setMsgAndRedirect($msg, "sysAdmin/order_kind/index/".$root."/".$nation);
			}	
		}else{
			
			$this->data["show_form"]=true;
			$this->data["action"] = site_url("sysAdmin/order_kind/create/".$root."/".$nation);
			$this->data["subtitle"] = $root==0  ? '新增付款方式主分類' : '【'.$this->kind->tb_sql("kind",$root).'】-新增子分類';
	...
	function edit($num,$nation){
		...
		if($row==NULL){
			...
			$this -> _setMsgAndRedirect($msg,"sysAdmin/order_kind/index");
		}else{		
			if($_POST){	//修改資料
				$parameter=array();
				$colSql="kind";
				...
				$this -> _setMsgAndRedirect($msg,"sysAdmin/order_kind/index/".$row["root"]."/".$row["nation"]);
				
			}else{
				
				
				$this->data["show_form"]=true;
				$this->data["action"]=site_url("sysAdmin/order_kind/edit/".$row["num"]."/".$nation);
				$this->data["maintitle"]=$this->getsubtitle();	//取得當前頁面名稱	
				$this->data["row"]=$row;	
				$this -> data["body"] = $this -> load -> view("sysAdmin/order/kind", $this -> data,true); 
				$this -> load -> view("sysAdmin/main", $this -> data);	
	...
	function delete($num){
		...
		if($row==NULL){
			$msg=array("type" => "warning", 'title' => '查無資料！');
			$this -> _setMsgAndRedirect($msg,"sysAdmin/page_kind/index");
		}else{
			$sqlStr="delete from [myTable] where num=?";
			$this->kind->sqlExc($sqlStr,array(':num'=>$num));
			$msg=array("type" => "success",'title' => '刪除成功！');
			$this -> _setMsgAndRedirect($msg,"sysAdmin/page_kind/index/".$row["root"]."/".$row["nation"]);
		}
		...
		if ($this -> input -> is_ajax_request()) {
			...
			if($rowAll!=NULL){
				foreach($rowAll as $row){
					...
					$options["tags"]=array('<a href="'.site_url("sysAdmin/order_kind/edit/".$row["num"]."/".$row["nation"]).'" class="btn btn-primary btn-xs" title="編輯"><span class="glyphicon glyphicon-pencil"></span></a>');
					
					if($level < $this->kindLevel){
						array_push($options['tags'],'<a href="'.site_url("sysAdmin/order_kind/create/".$row["num"]."/".$row["nation"]).'" class="btn btn-primary btn-xs" title="新增子分類"><span class="glyphicon glyphicon-plus"></span></a>');	
					}
	...
	function load_child_view($root,$level=2){
		...
		if($rowAll!=NULL){
			foreach($rowAll as $row){
				...
				$options["text"]=$row["kind"];
				$options["tags"]=array(
									'<a href="'.site_url("sysAdmin/page_kind/edit/".$row["num"]."/".$row["nation"]).'" class="btn btn-primary btn-xs" title="編輯"><span class="glyphicon glyphicon-pencil"></span></a>'
				);	
				if($level < $this->kindLevel){
					array_push($options['tags'],'<a href="'.site_url("sysAdmin/page_kind/create/".$row["num"]."/".$row["nation"]).'" class="btn btn-primary btn-xs" title="新增子分類"><span class="glyphicon glyphicon-plus"></span></a>');	
				}
				$options["href"]="javascript:void(0)";
				if($this->hasChild($row["num"]) && $level < $this->kindLevel){	//子節點
					$options["nodes"]=$this->load_child_view($row["num"],($level+1));
	...
	function myRange($root=0,$nation=NULL){		
		...
		$rowAll=$this->kind->sqlRowList($sqlStr,$parameter,'page_kind');
		
		$this->data["rowAll"]=$rowAll;		
		
		$this -> data["body"] = $this -> load -> view("sysAdmin/page/range", $this -> data,true); 
		$this -> load -> view("sysAdmin/main", $this -> data);	
	}
	//排序
	function load_view2(){
		if ($this -> input -> is_ajax_request()) {
			...
			if($rowAll!=NULL){
				foreach($rowAll as $row){
					$options=array();
					$options["text"]=$row["kind"];
					$options["href"]=site_url("sysAdmin/page_kind/myRange/".$row["num"]."/".(!$this->input->post('nation') ? "" : $this->input->post('nation',true)));
					if($this->hasChild($row["num"]) && $level < $this->kindLevel){	//子節點
						$options["nodes"]=$this->load_child_view2($row["num"],($level+1),$this->input->post('nation',true));
					}
	...
	//排序下一層
	function load_child_view2($root,$level=2,$nation){
		$data=array();
		$parameter=array();
		$sqlStr="select * from [myTable] where `root`=?";
		$parameter[":root"]=$root;
		...
		if($rowAll!=NULL){
			foreach($rowAll as $row){
				...
				$options["href"]=site_url("sysAdmin/page_kind/myRange/".$row["num"]."/".(empty($nation) ? "" : $nation));
				if($this->hasChild($row["num"]) && $level < $this->kindLevel){	//子節點
					$options["nodes"]=$this->load_child_view($row["num"],($level+1));
				}
	...
	function AjaxRange(){
		if($this->input->is_ajax_request()){
			...
			for ($i=0;$i<count($num);$i++){
				...
				$this->kind->sqlExc($sqlStr,$parameter,'page_kind');	
			}
		}
	}
