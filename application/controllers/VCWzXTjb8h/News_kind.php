...
class News_kind extends Core_controller{
	
	private $kindLevel=1;	//分類層數
	public function __construct(){
		parent::__construct();
		
		$this -> load -> model("sysAdmin/news_kind_model", "kind");
	...
	public function index($root = 0,$nation=NULL){		
		...
		$this->data["toptitle"]=$this->getsubtitle();	//取得當前頁面名稱
					
		$this -> data["body"] = $this -> load -> view("sysAdmin/news/kind", $this -> data,true);   
	...
	function create($root=0,$nation=NULL){	//新增選項	
		...
		if($_POST){	//新增資料
			...
			if(!$this->kind->sqlExc($sqlStr,$parameter)){
				...
				$this -> _setMsgAndRedirect($msg, "sysAdmin/news_kind/index/".$root."/".$nation);
			}	
		}else{
			
			$this->data["show_form"]=true;
			$this->data["action"] = site_url("sysAdmin/news_kind/create/".$root."/".$nation);
			$this->data["toptitle"]=$this->getsubtitle();	//取得當前頁面名稱
			$this->data["subtitle"] = $root==0  ? '新增訊息主分類' : '【'.$this->kind->tb_sql("kind",$root).'】-新增子分類';
	...
	function edit($num,$nation){
		...
		if($row==NULL){
			...
			$this -> _setMsgAndRedirect($msg,SYSTEM_URL."/news_kind/index");
		}else{		
			if($_POST){	//修改資料
				$parameter=array();
				$colSql="kind";
				...
				$this -> _setMsgAndRedirect($msg,SYSTEM_URL."/news_kind/index/".$row["root"]."/".$row["nation"]);
				
			}else{
				
				$this->data["toptitle"]=$this->getsubtitle();	//取得當前頁面名稱
				$this->data["show_form"]=true;
				$this->data["action"]=site_url(SYSTEM_URL."/news_kind/edit/".$row["num"]."/".$nation);
				$this->data["row"]=$row;	
				$this -> data["body"] = $this -> load -> view("sysAdmin/news/kind", $this -> data,true); 
				$this -> load -> view("sysAdmin/main", $this -> data);	
		...
		if ($this -> input -> is_ajax_request()) {
			...
			if($rowAll!=NULL){
				foreach($rowAll as $row){
					...
					$options["tags"]=array('<a href="'.site_url(SYSTEM_URL."/news_kind/edit/".$row["num"]."/".$row["nation"]).'" class="btn btn-primary btn-xs" title="編輯"><span class="glyphicon glyphicon-pencil"></span></a>');
					
					if($level < $this->kindLevel){
						array_push($options['tags'],'<a href="'.site_url(SYSTEM_URL."/news_kind/create/".$row["num"]."/".$row["nation"]).'" class="btn btn-primary btn-xs" title="新增子分類"><span class="glyphicon glyphicon-plus"></span></a>');	
					}
	...
	function load_child_view($root,$level=2){
		...
		if($rowAll!=NULL){
			foreach($rowAll as $row){
				...
				$options["text"]=$row["kind"];
				$options["tags"]=array(
									'<a href="'.site_url(SYSTEM_URL."/news_kind/edit/".$row["num"]."/".$row["nation"]).'" class="btn btn-primary btn-xs" title="編輯"><span class="glyphicon glyphicon-pencil"></span></a>'
				);	
				if($level < $this->kindLevel){
					array_push($options['tags'],'<a href="'.site_url(SYSTEM_URL."/news_kind/create/".$row["num"]."/".$row["nation"]).'" class="btn btn-primary btn-xs" title="新增子分類"><span class="glyphicon glyphicon-plus"></span></a>');	
				}
	...
	function load_child_view2($root,$level=2,$nation){
		...
		if($rowAll!=NULL){
			foreach($rowAll as $row){
				...
				$options["href"]=site_url(SYSTEM_URL."/news_kind/myRange/".$row["num"]."/".(empty($nation) ? "" : $nation));
				if($this->hasChild($row["num"]) && $level < $this->kindLevel){	//子節點
					$options["nodes"]=$this->load_child_view($row["num"],($level+1));
				}
