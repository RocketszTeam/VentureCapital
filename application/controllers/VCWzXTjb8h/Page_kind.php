...
class Page_kind extends Core_controller{
	
	private $kindLevel=1;	//分類層數
	public function __construct(){
		parent::__construct();
		
		$this -> load -> model("sysAdmin/page_kind_model", "kind");
	...
	public function index($root = 0,$nation=NULL){		
		...
		$this->data["action"] = site_url("sysAdmin/page_kind/create/".(empty($root) ? "" : $root)."/".(empty($nation) ? "" : $nation));
		$this->data["subtitle"] = '新增頁面主分類';
		$this->data["maintitle"]=$this->getsubtitle();	//取得當前頁面名稱			
		$this -> data["body"] = $this -> load -> view("sysAdmin/page/kind", $this -> data,true);   
	...
	function create($root=0,$nation=NULL){	//新增選項	
		...
		if($_POST){	//新增資料
			...
			$parameter=array();
			$colSql="nation,kind,admin_num,root,range,word,view";
			...
			$parameter[":word"]=$this->input->post("word");
			$parameter[":view"]=$this->input->post("view",true);
			if(!$this->kind->sqlExc($sqlStr,$parameter)){
				...
				$this -> _setMsgAndRedirect($msg, "sysAdmin/page_kind/index/".$root."/".$nation);
			}	
		}else{
			
			$this->data["show_form"]=true;
			$this->data["action"] = site_url("sysAdmin/page_kind/create/".$root."/".$nation);
			$this->data["subtitle"] = $root==0  ? '新增頁面主分類' : '【'.$this->kind->tb_sql("kind",$root).'】-新增子分類';
			$this->data["maintitle"]=$this->getsubtitle();	//取得當前頁面名稱	
			$this -> data["body"] = $this -> load -> view("sysAdmin/page/kind", $this -> data,true); 
			$this -> load -> view("sysAdmin/main", $this -> data);	
		}
	...
	function edit($num,$nation){
		...
		if($row==NULL){
			...
			if($_POST){	//修改資料
				...
				$this->data["show_form"]=true;
				$this->data["action"]=site_url("sysAdmin/page_kind/edit/".$row["num"]."/".$nation);
				$this->data["maintitle"]=$this->getsubtitle();	//取得當前頁面名稱	
				$this->data["row"]=$row;	
				$this -> data["body"] = $this -> load -> view("sysAdmin/page/kind", $this -> data,true); 
				$this -> load -> view("sysAdmin/main", $this -> data);	
		...
		if ($this -> input -> is_ajax_request()) {
			...
			if($rowAll!=NULL){
				foreach($rowAll as $row){
					...
					$options["tags"]=array('<a href="'.site_url("sysAdmin/page_kind/edit/".$row["num"]."/".$row["nation"]).'" class="btn btn-primary btn-xs" title="編輯"><span class="glyphicon glyphicon-pencil"></span></a>');
