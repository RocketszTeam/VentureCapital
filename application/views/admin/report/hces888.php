<script type="text/JavaScript">
	$(function(){
		...
		$('#yeWeek').click(function(){	//本週
			$('#find7').val($(this).attr('data-d1') + ' 00:00:00');
			$('#find8').val($(this).attr('data-d2') + ' 23:59:00');
		});
		...
		$('#auto_report').click(function(){
			...
			//86400000 1天的時間
			if(Date.parse($('#find8').val()) - Date.parse($('#find7').val()) > (86400000 * 1)){
				modalMsg("補帳日期區間不得超過1天！");
				//$('#find8').focus();
				return false;
			}
			...
			$.ajax({
				type: "POST",
				url: CI_URL + "report/Hces888_report/auto_report",
				cache: false,
				dataType:"json",
