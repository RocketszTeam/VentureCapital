<script type="text/JavaScript">
	$(function(){
		...
		$('#auto_report').click(function(){
			...
			//86400000 1天的時間
			if(Date.parse($('#find8').val()) - Date.parse($('#find7').val()) > (86400000 * 31)){
				modalMsg("補帳日期區間不得超過1個月！");
				//$('#find8').focus();
				return false;
			}
			...
			$.ajax({
				type: "POST",
				url: CI_URL + "report/Super_report/auto_report",
				cache: false,
				dataType:"json",
				...
				if(htmlData.RntCode=='Y'){
					modalMsg(htmlData.Msg,(htmlData.data_count > 0 ? location.href : ''));
					//alert(htmlData.Msg);
