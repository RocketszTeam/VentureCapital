<script type="text/JavaScript">
	$(function(){
		...
		$('#auto_report').click(function(){
			...
			if(Date.parse($('#find8').val()) - Date.parse($('#find7').val()) > 60*60*1000){
				modalMsg("補帳日期區間不得大於一小時！");
				//$('#find8').focus();
				return false;
			}
			...
			$.ajax({
				type: "POST",
				url: CI_URL + "report/lotpro_report/auto_report",
				cache: false,
				dataType:"json",
    ...
    <tbody>
        ...
        if(isset($result)):
            ...
            foreach($result as $row):
				...
				$validAmountTotal+=$row["betAmount"];
