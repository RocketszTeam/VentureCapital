<script type="text/JavaScript">
	$(function(){
		...
		$('#auto_report').click(function(){
			...
			//86400000 1天的時間
			if(Date.parse($('#find8').val()) - Date.parse($('#find7').val()) > (60000 * 15)){
				modalMsg("補帳日期區間不得超過15分鐘！");
				//$('#find8').focus();
				return false;
			}
			...
			$.ajax({
				type: "POST",
				url: CI_URL + "report/Ameba_report/auto_report",
				cache: false,
				dataType:"json",
...
<table id="simple-table" class="table table-bordered table-hover table-responsive">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-center">0.00<?php //echo  number_format($row["validAmount"],2,'.',',')?></td>
            <td class="text-center">
				<?php
                    $spanClass='';
                    if($row["winOrLoss"] < 0){
