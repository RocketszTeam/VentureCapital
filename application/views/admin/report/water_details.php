    ...
    <tbody>
        ...
        if(isset($result)):
            ...
            foreach($result as $row):
				...
				$validAmountTotal+=$row["RealBetPoint"];
        ...
        <tr>
            ...
            <td class="text-center">
                ...
                <div class="red"><?php echo $gameType[$row["Gid"]]?><?php echo $GMidType[$row["GMid"]]?></div>
            </td>
            <td class="text-left">
				<?php if($row["Result"]=='1') echo '<span class="text-success">[已結]</span>'?>
                <?php if($row["Result"]=='0') echo '<span class="text-danger">[未結算]</span>'?>
            </td>
            <td class="text-center"><?php echo number_format($row["BetGold"],2,'.',',')?></td>
            <td class="text-center"><?php echo  number_format($row["RealBetPoint"],2,'.',',')?></td>
            <td class="text-center">
				<?php
                    $spanClass='';
                    if($row["WinGold"] < 0){
